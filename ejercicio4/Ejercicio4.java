				...
				if (!line.startsWith("#")) {
					...
					if (!inputMatrix.isEmpty()) {
						System.out.println(String.format("%s. %s", caseNumber,
								new Chess(inputMatrix).haveCheck()));
						inputMatrix.clear();
					}
...
class Bishop extends Piece {
	...
	public boolean isAttacking(Point otherPositions, Board board) {
		int dx;
		int dy;
		Point road = new Point(this.position);
		if(Math.abs(this.position.getX() - otherPositions.getX()) == Math.abs(this.position.getY() - otherPositions.getY())) {
			dy = this.position.getY() <otherPositions.getY()? 1 : -1;
			dx = this.position.getX() <otherPositions.getX()? 1 : -1;
		}else {
			return false;
		}
...
class Rook extends Piece {
	...
	public boolean isAttacking(Point otherPositions, Board board) {
		...
		int dy;
		Point road = new Point(this.position);
		if(this.position.getX() == otherPositions.getX()) {
			dx = 0;	
			dy = this.position.getY() <otherPositions.getY()? 1 : -1;
		}else if(this.position.getY() == otherPositions.getY()) {
			dy = 0;
			dx = this.position.getX() <otherPositions.getX()? 1 : -1;
		}else {
			return false;
...
class Queen extends Piece {
	...
	public boolean isAttacking(Point otherPositions, Board board) {
		...
		if(this.position.getX() == otherPositions.getX()) {
			...
			dy = 0;
			dx = this.position.getX() <otherPositions.getX()? 1 : -1;
		}else if(Math.abs(this.position.getX() - otherPositions.getX()) == Math.abs(this.position.getY() - otherPositions.getY())) {
			dy = this.position.getY() <otherPositions.getY()? 1 : -1;
			dx = this.position.getX() <otherPositions.getX()? 1 : -1;
		}else {
			return false;
